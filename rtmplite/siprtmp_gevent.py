    ...
    def messageReceived(self, msg):
        if (msg.type == Message.RPC or msg.type == Message.RPC3) and msg.streamId == 0:
            ...
            if msg.type == Message.RPC or msg.type == Message.RPC3:
                ...
                elif cmd.name == 'transfer':
                    self.onStreamTransfer(stream, cmd)
                elif cmd.name == 'closeStream':
...
class User(object):
    ...
    def __init__(self, sock, gateway, start=False):
	...
	self.proxy = gateway
        ...
        self.address = self.username = self.password = None
    ...
    def _createRegister(self, interval):
	    ...
	    '''if self.gateway:
                m.Route = rfc3261.Header('sip:' + self.gateway, 'Route')'''
    ...
    def createServer(self, request, uri, stack):
        ...
        ua = request.method in ['INVITE', 'BYE', 'ACK', 'MESSAGE', 'NOTIFY'] and rfc3261.UserAgent(self.stack, request) or None
    ...
    def receivedRequest(self, ua, request, stack):
        ...
        if hasattr(ua, 'queue') and ua.queue is not None:
            ...
            if request.body and self._userQueue:
                ...
                ua.sendResponse(405, 'Method not allowed')
        elif request.method == 'NOTIFY':   # a paging-mode instant message
            if request.body and self._userQueue:
                ua.sendResponse(200, 'OK')      # blindly accept the message
                self._userQueue.put(('notify', (request['Content-Type'].value.lower(), request.body)))
            else:
                ua.sendResponse(405, 'Method not allowed')
...
class Session(object):
    ...
    def refer(self, sipuri):
        if self.ua:
            ua = self.ua
            m = ua.createRequest('REFER')
            m['Refer-To'] = rfc3261.Header('text/plain', 'Refer-To')
            m.body = '<sip:%s>'%(sipuri)
            ua.sendRequest(m)
...
class Context(object):
    ...
    def rtmp_register(self, login=None, passwd='', display=None, rate='wideband'):
        ...
        if _debug: print options.gateway_ip
	...
	proxy = '"%s" <sip:%s>'%(display, options.gateway_ip) if display else 'sip:%s'%(options.gateway_ip)
        ...
        try:
            user = self.user = User(sock, proxy).start() # create SIP user. Ownership of sock is moved to User.
    ...
    def sip_notify(self, contenttype, body):
        try:
            if _debug: print 'sip-notify'
            self.client.call('notified', str(contenttype), str(body))
        except:
            if _debug: print '  exception in sip_notify', (sys and sys.exc_info() or None)
    ...
    def rtmp_transfer(self, sipuri):
        try:
            if _debug: print 'rtmp-transfer', sipuri
            ua = self.session.ua
            m = ua.createRequest('REFER')
            m['Refer-To'] = rfc3261.Header('<%s>'%(sipuri), 'Refer-To')
            m['Referred-By'] = rfc3261.Header('<%s>'%(sipuri), 'Referred-By')
            m['Event'] = rfc3261.Header('refer', 'Event')
            ua.sendRequest(m)
        except:
            if _debug: print '  exception in rtmp_transfer'; traceback.print_exc()
...
if __name__ == '__main__':
    ...
    parser = OptionParser(version='SVN $Revision: 158 $, $Date: 2012-07-23 02:59:38 -0700 (Mon, 23 Jul 2012) $'.replace('$', ''))
    ...
    parser.add_option('-g', '--gateway', dest='gateway_ip',    default=None, help='gateway ip address to send all SIP traffic e.g. kamailio')
